...
This module handles SQLite-based storage for the Grace memory system.
"""
...
class SQLiteStorage:
    """SQLite storage for persistent memory and conversation logging."""
    ...
    async def save_memory_stats(self, stats: Dict):
        ...
        await loop.run_in_executor(self.executor, _save_stats)
    ...
    async def add_memory(self, content: str, memory_type: MemoryType, 
        ...
        # Run in thread pool
        loop = asyncio.get_running_loop()
        memory_id = await loop.run_in_executor(self.executor, _add_memory)
    ...
    async def search_memories(self, query: str, limit: int = 50):
        ...
        # Run in thread pool
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(self.executor, _search_memories)
    ...
    async def log_conversation(self, entry: ConversationEntry):
        ...
        # Run in thread pool
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(self.executor, _log_conversation)
        ...
        def _archive_old_memories():
            try:
                with sqlite3.connect(self.long_term_db) as conn:
                    ...
                    cursor.execute(
                        "UPDATE memories SET archived = 1 WHERE created_at < ?", (cutoff_date,)
                    )
                ...
                self.logger.error(f"Error archiving memories: {e}")
                return 0
                
        # Run in thread pool
        loop = asyncio.get_running_loop()
        archived_count = await loop.run_in_executor(self.executor, _archive_old_memories)
    ...
    async def get_conversation_by_id(self, conversation_id: int):
        ...
        # Run in thread pool
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(self.executor, _get_conversation)
    ...
    async def get_recent_conversations(self, limit: int = 10):
        ...
        # Run in thread pool
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(self.executor, _get_recent_conversations)
        ...
        def _search_conversations():
            try:
                with sqlite3.connect(self.conversation_db) as conn:
                    ...
                    rows = []
                    if fts_available:
                        # Use FTS for better search results - properly escape query
                        safe_query = query.replace('"', '""')  # Escape quotes for SQLite FTS
                        cursor.execute("""
                            SELECT c.* FROM conversation_search s
                            JOIN conversations c ON s.rowid = c.id
                            WHERE s MATCH ? AND c.archived = 0
                            ORDER BY c.timestamp DESC
                            LIMIT ?
                        """, (safe_query, limit))
                        rows = cursor.fetchall()
                    else:
                        # Fall back to LIKE search with parameterized query
                        like_pattern = f"%{query}%"
                        cursor.execute("""
                            SELECT * FROM conversations
                            WHERE (user_input LIKE ? OR model_response LIKE ?) AND archived = 0
                            ORDER BY timestamp DESC
                            LIMIT ?
    ...
    async def delete_memory(self, memory_id: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        # Run in thread pool
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(self.executor, _delete_memory)
